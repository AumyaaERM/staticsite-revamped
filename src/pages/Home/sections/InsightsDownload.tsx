...
export const InsightsDownloads: React.FC = () => {
  // Single source of truth — same hook the /insights page uses
  const { insights: allInsights, loading: isLoading, error } = useInsights();

  // Top 3 featured insights
  const insights = allInsights.filter((insight) => insight.featured).slice(0, 3);
            ...
            {insights.map((insight, i) => (
              ...
              >
                ...
                <div className="hidden sm:flex flex-shrink-0 w-36 md:w-44 lg:w-52 rounded-xl overflow-hidden self-stretch">
                  <img
                    src={insight.image || fallbackImages[i % fallbackImages.length]}
                    alt={insight.title}
